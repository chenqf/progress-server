...
class Sql{
    ...
    orderByASC(column){
        this.orderData = [...(this.orderData || []),{column:conversionKey(column),by:'ASC'}];
        return this;
    }
    orderByDESC(column){
        this.orderData = [...(this.orderData || []),{column:conversionKey(column),by:'DESC'}];
        return this;
    }
    toOrder(){
    ...
    toUpdate(){
        ...
        for(let key in this.data){
            ...
            list.push(`\`${conversionKey(key)}\` = '${value}'`);
        }
        return list.join(' , ');
    }
}
